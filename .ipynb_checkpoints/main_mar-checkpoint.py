...
def get_args_parser():
    ...
    parser.add_argument('--time_series_length', default=500, type=int,
                        help='time series sequence length')
    parser.add_argument('--timeseries_channels', default=12, type=int,
                        help='number of time series channels')
    ...
    parser.add_argument('--sd3_model_path', default="/projects/besp/BiosignalGen_zitao/models--stabilityai--stable-diffusion-3-medium-diffusers/snapshots/ea42f8cef0f178587cf766dc8129abd379c90671", type=str,
                        help='Path to SD3 model for text conditioning')

    # Generation parameters
    parser.add_argument('--num_iter', default=64, type=int,
                        help='number of autoregressive iterations to generate a time series')
    ...
    parser.add_argument('--eval_bsz', type=int, default=50, help='generation batch size')
    ...
    # Dataset parameters
    parser.add_argument('--data_path', default='./data/timeseries', type=str,
                        help='dataset path')
    parser.add_argument('--class_num', default=10, type=int)

    parser.add_argument('--output_dir', default='/projects/besp/shared_data/MAR/results',
                        help='path where to save, empty for no saving')
    parser.add_argument('--log_dir', default="/projects/besp/shared_data/MAR/results",
...
def main(args):
    ...
    if global_rank == 0 and args.log_dir is not None:
        ...
        log_writer = None
#     # TODO: Replace with your time series dataset
#     # For now, create a dummy dataset
#     class DummyTimeSeriesDataset:
#         def __init__(self, num_samples=1000, time_series_length=128, channels=8, num_classes=10, use_text_conditioning=False):
...
#             data = torch.randn(self.channels, self.time_series_length)
#             if self.use_text_conditioning:
#                 # Return text prompt instead of class label
#                 label = f"time series class {torch.randint(0, self.num_classes, (1,)).item()}"
#             else:
#                 # Return class label
#                 label = torch.randint(0, self.num_classes, (1,)).item()
#             return data, label
    
#     dataset_train = DummyTimeSeriesDataset(
#         num_samples=1000,
#         time_series_length=args.time_series_length,
#         channels=args.timeseries_channels,
#         num_classes=args.class_num,
#         use_text_conditioning=args.use_text_conditioning
#     )
    dataset_train = ECG_Text_Dataset(
        split="test",
    ...
    # define the mar model
    model = mar.__dict__[args.model](
        time_series_length=args.time_series_length,
        timeseries_channels=args.timeseries_channels,
        patch_size=args.patch_size,
        vae_embed_dim=32,  # Keep for internal compatibility
        ...
        use_text_conditioning=args.use_text_conditioning,
        sd3_model_path=args.sd3_model_path,
    )
    ...
    # Synchronize after model initialization to prevent distributed deadlocks
    if args.distributed:
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        if args.online_eval and (epoch % args.eval_freq == 0 or epoch + 1 == args.epochs):
            torch.cuda.empty_cache()
            evaluate(model_without_ddp, ema_params, args, epoch, batch_size=args.eval_bsz, log_writer=log_writer,
                     cfg=1.0, use_ema=True)
            if not (args.cfg == 1.0 or args.cfg == 0.0):
                evaluate(model_without_ddp, ema_params, args, epoch, batch_size=args.eval_bsz // 2,
